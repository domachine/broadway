...
App.prototype.use = function (plugin, options, callback) {
  ...
  //
  // Determine if there are any required `npm` deps for this plugin
  //
  if(typeof this.plugins[name].dependencies === "object") {
    var deps = this.plugins[name].dependencies;
    //
    // TODO: improve this code, perhaps require 'npm' package in broadway itself...
    //

    //
    // Deps were find, determine if they are available
    //
    Object.keys(deps).forEach(function(d){
      try {
        require(d);
      } catch (err) {
        throw new Error('Error: plugin ' + self.plugins[name].name + ' is missing dep ' + ( d + '@' + deps[d]));
      }
    });
  }
